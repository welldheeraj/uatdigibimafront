...
export default function SummaryCard({
  ...
  const priceChangeReason =
    "The PIN code in your address is different from the PIN code you chose while taking quote. Hence, the total premium is revised.";
        ...
        <>
          ...
          <div className="space-y-1 text-gray-700">
            {Object.entries(selectedAddons).map(([k, val]) => {
              ...
              if (isOpdVariant(valueStr)) {
              ...
              else if (keyStr === "opd" && isOpdVariant(valueStr)) {
                displayKey = "opd";
                rightSide = formatPrice(addons?.opd ?? 0);
              }
              else if (valueStr === "ped" || keyStr === "ped") {
                displayKey = "ped";
                ...
                displayKey = valueStr;
                rightSide = formatPrice(addons?.[valueStr] ?? addons?.[keyStr] ?? 0);
              }
