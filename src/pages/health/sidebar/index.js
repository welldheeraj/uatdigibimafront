...
if (res?.status) {
  ...
  if (String(displayedPincode) !== String(pincode)) {
    // 🔥 Clear both plan caches when pincode changes
    await Promise.all([
      deleteDBData(constant.DBSTORE.HEALTH.PLANS.HEALTHPLANDATA),
      deleteDBData(constant.DBSTORE.HEALTH.PLANS.HEALTHPLANVENDOR)
    ]);
    console.log("✅ Cleared plan caches (pincode changed)");
  }
  
       setTimeout(() => {
    router.push(`${constant.ROUTES.HEALTH.PLANS}?refresh=${Date.now()}`);
  }, 300);
