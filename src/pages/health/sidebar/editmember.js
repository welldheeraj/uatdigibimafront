...
export default function InsureSidebarComponent({ onClose }) {
  ...
  useEffect(() => {
    const getInsureData = async () => {
      try {
        ...
        if (!res.status) return showError("Failed to fetch data.");

        if (res.gender) setGender(res.gender.toLowerCase());

        const apiData = res.data || [];
        const updatedMembers = [
          { name: "self", age: apiData.find((m) => m.name === "self")?.age || "" },
          { name: gender === "male" ? "wife" : "husband", age: apiData.find((m) => m.name === (gender === "male" ? "wife" : "husband"))?.age || "" },
          ...["father","mother","grandfather","grandmother","fatherinlaw","motherinlaw"].map((m) => ({
            name: m,
            age: apiData.find((x) => x.name === m)?.age || "",
          })),
        ];

        const childData = apiData
          .filter((m) => m.name === "Son" || m.name === "Daughter")
          .map((m) => ({ name: m.name, age: m.age }));

        setChildren(childData);
        setIsChildChecked(childData.length > 0);
        setMembers(updatedMembers);

        const selected = apiData.filter((m) => m.name !== "Son" && m.name !== "Daughter").map((m) => m.name);
  ...
  const addChild = () => {
    if (children.length < maxChildren) setChildren([...children, { name: "", age: "" }]);
    else showError("Maximum Four Children Allowed");
  };

  const removeChild = () => setChildren(children.slice(0, -1));
  ...
  const ageChange = (name, age) => {
    setMembers((prev) => prev.map((m) => (m.name === name ? { ...m, age } : m)));
  };
  ...
  const handleSubmit = async () => {
    ...
    if (isChildChecked) {
      for (let i = 0; i < children.length; i++) {
        ...
        if (ageNum < 1 || ageNum > 24) return showError(`Child ${i + 1}: Age must be between 1 and 24`);
      }
    }
    ...
    try {
      ...
      if (response.status) {
        ...
        queryClient.invalidateQueries(["vendorPlans"]);
        router.push(constant.ROUTES.HEALTH.PLANS);
        onClose();
      } else showError(response.error || "Failed to submit data. Please try again.");
    } catch {
    ...
    <div className="w-full h-full px-1 py-4 overflow-y-auto">
      ...
      <div className="space-y-4">
        {["self", "wife", "husband"].map((name) => {
          ...
          return member ? <MemberCard key={member.name} {...{ member, selectedMembers, handleToggle, ageChange }} /> : null;
        })}
        ...
        <div className="flex justify-center">
          <button onClick={handleSubmit} className="w-[96%] text-center py-2 thmbtn">Continue</button>
        </div>
      </div>
    </div>
  );
      ...
      <div onClick={toggleChildCheckbox} className="flex items-center justify-between gap-2 bg-white px-4 py-3 rounded-xl text-black w-full relative border border-gray-400">
        ...
        {isChildChecked && (
          <div className="flex items-center space-x-2" onClick={(e) => e.stopPropagation()}>
            <button onClick={removeChild} type="button" disabled={children.length === 0} className="thmbtn rounded"><HiMinus className="text-xl text-white" /></button>
            <span className="font-medium text-gray-700">{children.length}</span>
            <button onClick={addChild} type="button" disabled={children.length >= maxChildren} className="thmbtn rounded"><HiPlus className="text-xl text-white" /></button>
          </div>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 gap-4 mt-4">
          {children.map((child, index) => (
            <div key={index} className="flex items-center gap-3 border border-gray-400 p-2 rounded-xl bg-white shadow-sm">
              <select className="w-1/2 border border-gray-400 text-sm font-medium text-gray-800 cursor-pointer px-3 py-2 rounded-md" value={child.name} onChange={(e) => childChange(index, "name", e.target.value)}>
                <option value="">Select Child</option>
                <option value="Son">Son</option>
                <option value="Daughter">Daughter</option>
              </select>
              <select className="w-1/2 border border-gray-400 text-sm font-medium text-gray-800 cursor-pointer px-3 py-2 rounded-md" value={child.age} onChange={(e) => childChange(index, "age", e.target.value)}>
                <option value="">Age</option>
                {Array.from({ length: 24 }, (_, i) => i + 1).map((age) => <option key={age} value={age}>{age}</option>)}
              </select>
            </div>
          ))}
        </div>
