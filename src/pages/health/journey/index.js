...
import validateKycStep  from "./stepper.js";
...
const validateFormStepOne = async () => {
  // Step 1: Validate KYC section first
  const result = validateKycStep(kycType, step1Form.getValues(), proofs);
  if (result !== true) {
    showError(result);
    return false;
  }

  // Step 2: Then validate the rest of the form (e.g., contact details, address)
  const isValid = await validateFields(step1Form);

  return isValid;
};

  const validateFormStepTwo = async () => await validateFields(step2Form, ["address2"]);
  ...
  // Placeholder verification handlers for PAN, Aadhar, and others
  const handleVerifyPan = () => {};
  const handleVerifyAadhar = () => {};
  const handleVerifyOther = () => {};
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-6">
        <div className="flex-1 bg-white rounded-[32px] shadow p-8 shadow">
            ...
            {steps.map((label, i) => {
              ...
              return (
                ...
                >
                  ...
                  {sn !== steps.length && (
                    <div className={`flex-1 h-0.5 mx-2 ${done ? "thmbtn" : "bg-gray-200"}`} />
                  )}
                </div>
              );
            })}
          ...
          <div className="mt-10">
            ...
            {currentStep === 3 && (
              <StepThreeForm step3Form={step3Form} onSubmitStep={onSubmitStep} />
            )}
            {currentStep === 4 && (
