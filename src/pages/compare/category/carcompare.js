...
const getPlanKey = (p, i=0) =>
  `${String(p?.productname || p?.title || "")}|${String(p?.idv || "")}|${String(p?.premium || "")}|${i}`;
...
// normalize motor-ish shapes (car)
const normalize = (p) => ({
  productname: p.productname || p.title || "",
  premium: p.premium ?? p.minrange ?? p.price ?? "",
  idv: p.idv ?? p.IDV ?? "",
  features: {
    zeroDep: p.features?.zeroDep ?? p.zeroDep ?? p.addons?.["Zero Depreciation"] ?? p.addons?.zeroDep ?? false,
    ncb: p.features?.ncb ?? p.ncbPercent ?? p.ncb ?? p.addons?.ncbPercent ?? undefined,
  },
  addons: Array.isArray(p.addons) ? p.addons :
          (typeof p.addons === "object" ? Object.keys(p.addons || {}) : []),
...
const buildRows = ({ onOpenAddons, highestIDV }) => ([
  { key: "idv", label: "IDV", render: (p) => (p.idv ? `₹${formatINR(p.idv)}` : "—"), highlight: (p) => toNum(p.idv) === highestIDV, pretty: (p) => (p.idv ? `₹${formatINR(p.idv)}` : "—") },
  { key: "zeroDep", label: "Zero Depreciation", render: (p) => (p.features?.zeroDep ? "Yes" : "No"),
    pretty: (p) => <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${p.features?.zeroDep ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>{p.features?.zeroDep ? "Yes" : "No"}</span> },
  { key: "ncb", label: "NCB",
    render: (p) => { const ncb = p.features?.ncb; if (ncb == null || ncb === "") return "—"; const num = toNum(ncb); return num ? `${num}%` : String(ncb); },
    pretty: (p) => { const ncb = p.features?.ncb; if (ncb == null || ncb === "") return "—"; const num = toNum(ncb); return num ? `${num}%` : String(ncb); } },
...
export default function CarCompare({ plans = [] }) {
  ...
  const highestIDV = useMemo(() => {
    const nums = normalized.map((p) => toNum(p.idv));
  ...
  const rows = useMemo(() => buildRows({ onOpenAddons: (p) => setAddonsModal({ open: true, plan: p }), highestIDV }), [highestIDV]);
  ...
  const handleProceedBack = () => {
    ...
    const fallback = (typeof window !== "undefined" && sessionStorage.getItem("compareBack")) || "/motor";
        ...
        <div className="overflow-x-auto rounded-3xl shadow-2xl bg-white/80 backdrop-blur-lg border border-slate-200">
          <table className="min-w-full text-sm border-separate border-spacing-0">
            ...
            <tbody>
              ...
              {buildRows({ onOpenAddons: (p) => setAddonsModal({ open: true, plan: p }), highestIDV })
        ...
        <div className="mx-auto max-w-7xl px-6 pb-4">
          <div className="rounded-2xl border border-indigo-200 bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/80 shadow-2xl px-6 py-4 flex items-center justify-between">
            <div className="text-sm md:text-base font-semibold text-slate-800">
              Secure your vehicle with the right protection.
            ...
            return (
              <div key={getPlanKey(p, i)} className="bg-white/90 backdrop-blur-md rounded-2xl shadow-lg border border-slate-200 p-5 relative">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <p><span className="font-semibold">IDV:</span> {p.idv ? `₹${formatINR(p.idv)}` : "—"}</p>
                  <p><span className="font-semibold">Zero Depreciation:</span> {p.features?.zeroDep ? "Yes" : "No"}</p>
                  <p>
